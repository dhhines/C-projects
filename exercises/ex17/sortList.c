 ...
 * @file sortList.c
 * 
 * Program that illustrates the use of Preprocessor macros in order
 * to define functions and variables inline prior to code execution.
 *
 * The final program takes an input file from the stdin and uses
 * the type provided to create a list then sort that list and
 * finally print the list to stdout
 */
...
#define CHECK_CAP(type, name) \
  if(name##_len >= name##_cap) { \
    name##_cap *= 2;  \
    name = (type *) realloc( name, sizeof(double) * (name##_cap)); \
  }
