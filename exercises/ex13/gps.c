 ...
 * @author David Hines
 * @file gps.c
 * Program to learn structs and how to interact with them when combined
 * with arrays (i.e. arrays of structs).  This program also takes advantage
 * of dynamic memory to create the address space for the array of structs
 * and then provides practice for pass by value and reference of structs.
 */
...
void readCity( FILE *fp, struct City *cty ) {
  ...
  fscanf(fp, "%s %lf %lf", cty->name, &cty->latitutde, &cty->longitude);
...
int main( int argc, char *argv[] )
{
  ...
  while ( scanf( "%lf%lf", &lat, &lon ) == 2 ) {
    ...
    char *nearest;
    for (int i = 0; i < n; i++){
      if ((distance = globalDistance(lat, lon, cities[i].latitutde, cities[i].longitude)) < closest || closest == 0.0){  
        nearest = cities[i].name;
        closest = distance;
      }
    }
